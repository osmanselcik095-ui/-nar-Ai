...
function getAI() {
  ...
  if (!apiKey || apiKey === "MY_GEMINI_API_KEY") {
    throw new Error("GEMINI_API_KEY is missing. Please add your Gemini API Key in the Vercel Dashboard (Environment Variables).");
  }
  if (!aiInstance) {
...
app.post("/api/tts", async (req, res) => {
  ...
  try {
    ...
    return res.status(error.message?.includes("GEMINI_API_KEY") ? 403 : 500).json({
      error: error.message || "Ses üretilirken sistemsel bir hata oluştu.",
    });
  }
});
